...
namespace MakeMKV_Title_Decoder.Data
{
	public abstract class LoadedStream
	{
        ...
        public MediaAnalysis FFProbeInfo { get; private set; }

		public List<LoadedTrack> Tracks { get; private set; }
		...
		internal string? TryMatchRenameData(ClipRename renameData, string root)
		{
            this.TrackMatches = null;

            bool remux = false;
            if (renameData.Remux != null)
            {
                string? remuxResult = RemuxClip(root, renameData);
                if (remuxResult != null)
                {
                    return remuxResult;
                }
            ...
            if (!remux)
            {
                error = this.Identity.Match(renameData.Identity);
            ...
            if (!remux && this.Tracks.Count != renameData.Tracks.Count) throw new Exception("Number of streams does not match.");
        ...
        // Attempts to remux the clip with the given remux commands.
        private string? RemuxClip(string root, ClipRename renameData) {
            string sourceFile = renameData.Remux.OriginalFile;
            string sourceFileFullPath = Path.Combine(root, sourceFile);
            string outputFile = Path.Combine("remux", $"{Path.Combine(Path.GetDirectoryName(sourceFile), Path.GetFileNameWithoutExtension(sourceFile))}.mkv");
            string outputFileFullPath = Path.Combine(root, outputFile);
            string logFileFullPath = Path.Combine(root, "remux", $"{Path.Combine(Path.GetDirectoryName(sourceFile), Path.GetFileNameWithoutExtension(sourceFile))}.log");

            FFMpeg ffmpeg = new FFMpeg(FileUtils.GetFFMpegExe());
            FFProbe ffprobe = new FFProbe(FileUtils.GetFFProbeExe());

            MediaAnalysis? analysis = ffprobe.Analyse(sourceFileFullPath);
            Debug.Assert(analysis != null);

            Directory.CreateDirectory(Path.GetDirectoryName(outputFileFullPath));
            OutputFile outputFileFFMpeg = new OutputFile(outputFileFullPath);

            FFMpegArgs args = renameData.Remux.GetCommand(ffmpeg, analysis, outputFileFFMpeg);
            string? error = null;
            TaskProgressViewerForm.Run((IProgress<SimpleProgress> progress) =>
            {
                args.SetOverwrite(true).SetLogPath(logFileFullPath).NotifyOnProgress(percent =>
                {
                    SimpleProgress prog = new((uint)percent, 100);
            ...
            if (error == null)
            {
                // Reload the tracks to make sure all data is captured correctly
                //renameData.Identity = new StreamIdentity(outputFile, DataSize.FromFile(outputFileFullPath).Value, ffprobe.Analyse(outputFileFullPath));
                this.LoadData(root, outputFile, ffprobe.Analyse(outputFileFullPath));
            }

            return error; // Indicates success when null
        }
    }
}
